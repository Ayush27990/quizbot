...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("approve_"):
        ...
        item = pending_questions.pop(qid, None)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith("http://") or text.startswith("https://"):
        ...
        if youtube_id:
            await update.message.reply_text("🎥 YouTube link detected! Fetching transcript...")
            transcript = await get_youtube_transcript(youtube_id)
            if not transcript:
                await update.message.reply_text("❌ Could not get transcript. Trying page content...")
                transcript = await fetch_url_content(text)
            if not transcript:
                await update.message.reply_text("❌ Could not extract content from YouTube video.")
                return
            ...
            await update.message.reply_text("🔗 Article URL detected! Fetching content...")
        ...
        await update.message.reply_text("💬 Forwarded MCQ detected! Processing...")
