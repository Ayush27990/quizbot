       ...
       async with httpx.AsyncClient(timeout=20) as client:
           ...
           for img in img_tags:
               ...
               if src.startswith("//"):
                   ...
                   continue
               skip_keywords = ["logo", "icon", "avatar", "banner", "social", "button", "arrow", "emoji", "spinner", "placeholder"]
               if any(k in src.lower() for k in skip_keywords):
                   continue
               medical_keywords = ["ecg", "ekg", "xray", "x-ray", "scan", "mri", "ct", "echo", "ultrasound", "wave", "rhythm", "lead", "chest", "cardiac", "heart", "wp-content/uploads"]
               if any(k in src.lower() for k in medical_keywords) or "wp-content" in src.lower():
                   img_urls.insert(0, src)
               else:
                   img_urls.append(src)
           for src in img_urls[:10]:
               try:
                   img_response = await client.get(src, headers=headers, follow_redirects=True, timeout=10)
                   content_type = img_response.headers.get("content-type", "")
                   if "image" in content_type and len(img_response.content) > 15000:
                       images.append(img_response.content)
                       logger.info(f"Found image: {src}")
               except Exception as e:
                   logger.warning(f"Could not fetch image {src}: {e}")
                   continue
               if len(images) >= 3:
                   break
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
   await update.message.reply_text(
       ...
       "Answer each question to get a detailed explanation!"
   )
